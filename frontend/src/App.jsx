...
import { io } from 'socket.io-client'; // 🚀 Import the WebSocket engine client link
import { 
...
// Static Asset Bundle Imports for Production Bundler Compilation
import websiteLogo from './assets/Website Logo.png';
import appIcon from './assets/App Icon.png';

// 🔗 LINK YOUR WEBSOCKET SERVER HIGHWAY NODE
// Local Testing: Use "http://localhost:5000"
// Production: Swap this out for your live Render/Railway URL (e.g., "https://steel-suvidha-api.onrender.com")
...
export default function App() {
  ...
  // ─── TICKER PRICES STATE: Initial baseline prices before third-party app stream syncs ───
  const [tickerPrices, setTickerPrices] = useState([
    { item: "TMT 8mm (TATA)", price: 62450, change: 1.2, up: true },
    { item: "TMT 12mm (JSW)", price: 58900, change: -0.4, up: false },
    { item: "Angle 40×5 (SAIL)", price: 51200, change: 0.8, up: true },
    { item: "Flat 25×3", price: 49500, change: -1.1, up: false },
    { item: "ISMC 100 Channel", price: 54100, change: 2.3, up: true },
    { item: "HR Sheet 10G", price: 66800, change: 0.2, up: true },
    { item: "Color Profile 0.45mm", price: 74200, change: -0.7, up: false }
  ]);

  const API_URL = 'http://localhost:5000/api/blogs';
  const PLAY_STORE_URL = "https://play.google.com/store/apps/details?id=com.steelsuvidha.app";

  // ─── 🚀 PRODUCTION WEBSOCKET EVENT LISTENER MATRIX ───
  useEffect(() => {
    // Open a persistent traffic channel to your Node backend wrapper server
    const socketConnection = io(BACKEND_WS_URL);

    console.log("System Status: Synchronizing real-time websocket listener...");

    // Catch the real-time payload broadcast from your backend webhook layer
    socketConnection.on('priceUpdate', (freshMarketData) => {
      console.log("Websocket Stream: Captured authentic third-party price updates:", freshMarketData);
      // Overwrite static display arrays with real-time market data instantly
      setTickerPrices(freshMarketData);
    });

    // Clean up: Automatically break the socket connection thread if the user closes the browser tab
    return () => {
  ...
  const fetchBlogs = async () => {
    try {
      ...
      console.warn("Backend API data pipeline offline. Running components securely on local memory.");
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F1F5F9] text-[#263238] selection:bg-[#E23744] selection:text-white antialiased font-poppins">
      ...
      {/* ─── HIGH PERFORMANCE FIXED STACKED HEADER NAVIGATION CHASSIS ─── */}
      <div className="sticky top-0 z-50 flex flex-col w-full shadow-xs">
        
        {/* LAYER A: BRAND BAR NAVBAR */}
        <header className="bg-white/95 backdrop-blur-md border-b border-slate-200 px-4 sm:px-8 py-4 flex items-center justify-between">
          ...
          <button 
            ...
            className="p-2 text-slate-600 lg:hidden hover:text-black focus:outline-hidden cursor-pointer"
            aria-label="Toggle Adaptive Navigation Tray Menu"
          >
            {isMobileMenuOpen ? <X size={24} /> : <Menu size={24} />}
          </button>
        </header>

        {/* LAYER B: SCROLLING LIVE WEBSOCKET PRICE TICKER */}
        <div className="w-full bg-[#0D1137] text-white overflow-hidden py-2.5 border-b border-slate-900 select-none z-40">
          <div className="relative w-full flex items-center">
            
            <div className="absolute left-0 top-0 bottom-0 bg-[#E23744] text-white px-4 flex items-center z-50 font-black tracking-wider text-[10px] uppercase shadow-md shadow-black/30">
              ...
              <div className="animate-marquee whitespace-nowrap flex items-center gap-12 text-xs font-bold font-mono">
                {[...tickerPrices, ...tickerPrices].map((t, idx) => (
                  <div key={idx} className="inline-flex items-center gap-2 tracking-wide">
                    ...
                    <span className="text-white font-mono">₹{t.price.toLocaleString('en-IN')}/MT</span>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-40 lg:hidden flex flex-col bg-white pt-28 px-6 animate-fade-in">
          <nav className="flex flex-col space-y-4 text-base font-bold tracking-wide">
            {menuNavigationItems.map((tab) => (
              <button
                key={tab.id}
                onClick={() => scrollToSection(tab.id)}
                className="w-full text-left py-3 px-4 rounded-xl border border-slate-100 bg-slate-50 text-slate-800"
              >
                {tab.label}
              </button>
            ))}
            ...
            <button
              onClick={() => { setIsMobileMenuOpen(false); setShowAdminPanel(true); }}
              className="w-full text-left py-3 px-4 rounded-xl bg-red-50 border border-red-100 text-[#E23744] font-bold flex items-center justify-between"
            >
              <span>Secure Operation Gateway</span>
              <Lock size={16} />
            </button>
          </nav>
        </div>
      ...
      <main className="flex-grow relative z-10 space-y-20">
        ...
        <div id="hero-section" className="w-full pt-8">
          <div className="max-w-7xl mx-auto px-6 sm:px-8 py-10 lg:py-24 grid lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 gap-4 font-poppins">
              <div className="bg-gradient-to-br from-slate-800 to-slate-900 text-white p-6 rounded-2xl border border-slate-700 shadow-xl flex flex-col justify-between h-48 relative overflow-hidden group">
        ...
        {/* SECTION 5: DIGITAL MENU FLIP-BOOK */}
        ...
        {/* SECTION 7: PUBLIC BLOG INSIGHTS */}
        ...
        {/* ─── ISOLATED OPERATIONS MODAL GATEWAY LAYER ─── */}
        {showAdminPanel && (
            ...
            <div className="bg-white w-full max-w-5xl rounded-2xl border border-slate-200 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
              <div className="p-5 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                <div className="flex items-center gap-2 text-slate-800">
                  ...
                  <h3 className="font-bold font-lora text-sm">{isAdminAuthenticated ? "Cluster Ledger Console Node: Active" : "System Cryptographic Gate Access Loop"}</h3>
                </div>
                <button onClick={() => { setShowAdminPanel(false); setIsAdminAuthenticated(false); setAdminPassphrase(''); }} className="text-xs font-bold text-slate-400 hover:text-slate-900 cursor-pointer">Terminate Connection</button>
              </div>
              ...
              {!isAdminAuthenticated ? (
                <div className="p-8 flex flex-col items-center justify-center space-y-6 max-w-md mx-auto my-12">
                  ...
                  <div className="text-center space-y-1"><h4 className="font-bold text-sm text-slate-900 font-lora">Authentication Token Mandated</h4><p className="text-xs text-slate-400">Provide the baseline administrative operational passcode loop.</p></div>
                  <form onSubmit={handleAdminAuthSubmit} className="w-full space-y-4">
                    <div className="relative">
                      <input type={isPassVisible ? "text" : "password"} value={adminPassphrase} onChange={(e) => setAdminPassphrase(e.target.value)} placeholder="Enter primary administrative passphrase..." className="w-full text-xs p-3.5 pl-4 pr-10 border border-slate-200 rounded-xl bg-[#F8FAFC] font-mono tracking-wide focus:outline-hidden focus:border-[#E23744]" required />
                      <button type="button" onClick={() => setIsPassVisible(!isPassVisible)} className="absolute right-3.5 top-1/2 transform -translate-y-1/2 text-slate-400 hover:text-slate-600 cursor-pointer" aria-label="Toggle Password Visibility Status String">{isPassVisible ? <EyeOff size={15} /> : <Eye size={15} />}</button>
                    </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="flex flex-col items-center md:items-end gap-2">
            ...
            <a 
              ...
              aria-label="Download Steel Suvidha Operations Software on Google Play Store"
            >
